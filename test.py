...
for index, row in data.iterrows():
    plasmid_list.append(AllPlasmid(plasmid_id=row[0], source=int(row[1]), topology=row[2], completeness=row[3], length=int(row[4]), gc_content = float(row[5]), host = row[6], mob_type = row[7], mobility = row[8], cluster = row[9], subcluster = row[10], unique_id=row[11]))
